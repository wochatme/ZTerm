...
- 第一个域是紧随消息包类型字节的连续的三个字节，它表示服务器的版本信息，目前固定为"100"，表示是1.0.0版本。
- 第二个域是66字节，该域为用户的公钥，由数字0-9和小写字母a-f表示。形式类似："03339a1c8fdb6aff46845e49d120e0400021e161b6341858585c2e25ca3d9c01ca"。
- 第三个域是文档的编号，16字节，由数字0-9和小写字母a-f表示。
- 第四个域64字节，保留。
...
- 第一个字节为消息包类型。对于本类型的消息包，该字节恒定为小写的k。
- 第一个域是紧随消息包类型字节的连续的三个字节，它表示服务器的版本信息，目前固定为"100"，表示是1.0.0版本。
- 第二个域是65字节，该域为用户的公钥，由数字0-9和小写字母a-f表示。形式类似："3339a1c8fdb6aff46845e49d120e0400021e161b6341858585c2e25ca3d9c01ca"。
- 第三个域是65字节，该域为服务器的公钥，由数字0-9和小写字母a-f表示。形式类似："267e45777b1e209cb04937b316e59f4d1a0f5bdd43a1a8489fba702ce9085662b"。 
- 第四个域是文档的编号，16字节，由数字0-9和小写字母a-f表示。
- 第五个域是文档的密钥，64字节，由数字0-9和小写字母a-f表示。 如果拒绝提供密钥，该域全部为’0‘，就是64个字符'0'组成。
- 第六个域是文档的标题.可变长度.
...
加密的文档格式解释如下：
- 第一域4字节表示最终文件的尺寸，按照小端格式存储。
- 第二个域32字节表示文件的编号。它是第3、4、5、6域的最终数据的哈希值，也可以校验文件是否损坏。
- 第三个域32字节是原始数据的SHA256的哈希值，用于校验文件内容是否损坏。
- 第四个域是原始文件的长度，单位是字节，按小端格式保存。
- 第五个域是4字节，表示文件的类型。对于文本文件，就是"TXT@"，对于我们的视频格式，是"RDO@"。在加密之前，该域和第四个域的4字节一一进行异或处理。
- 第六个域是原始数据，可能被压缩了，譬如所有的文本文件都被压缩了。

第3、4、5、6域的数据采用Chacha20的流加密算法进行加密。加密的密钥K为32字节，单独保存在数据库中。当用户索要加密文档时，K被Kp加密为KK。Kp为32字节，它是由服务器的私钥和用户的公钥计算出来的。Kp加密K的算法是AES256。
```
KK = g(K, Kp)
```
